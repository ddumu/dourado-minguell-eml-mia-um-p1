...
def q_learning(
    ...
    for t in tqdm(range(num_episodes)):
        ...
        while not done:
            # Acción elegida por la política de comportamiento (epsilon-greedy)
            ...
            # Añadir la recompesa esperada si el episodio no ha terminado
            if not done:
                reward += discount_factor * np.max(Q[new_state])

            # Actualizar las estimaciones para el par estado-acción
            Q[state, behaviour_action] += alpha * (reward - Q[state, behaviour_action])
